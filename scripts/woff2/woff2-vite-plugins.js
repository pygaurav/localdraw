...
/**
 * Custom vite plugin for auto-prefixing `EXCALIDRAW_ASSET_PATH` woff2 fonts in `localdraw-app`.
 *
...
module.exports.woff2BrowserPlugin = () => {
  ...
  return {
    ...
    transform(code, id) {
      ...
      // but at the same time can't be easily prefixed with the `EXCALIDRAW_ASSET_PATH` only for the `localdraw-app`
      if (!isDev && id.endsWith("/localdraw/fonts/fonts.css")) {
        return `/* WARN: The following content is generated during localdraw-app build */
      ...
      if (!isDev && id.endsWith("localdraw-app/index.html")) {
        return code.replace(
