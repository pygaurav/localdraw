import { promiseTry, LOCAL_FONT_PROTOCOL } from "@localdraw/common";
...
export class ExcalidrawFontFace {
  ...
  private static createUrls(uri: string): URL[] | DataURL[] {
    ...
    if (typeof window.LOCALDRAW_ASSET_PATH === "string") {
      const normalizedBaseUrl = this.normalizeBaseUrl(
        window.LOCALDRAW_ASSET_PATH,
      );

      urls.push(new URL(assetUrl, normalizedBaseUrl));
    } else if (Array.isArray(window.LOCALDRAW_ASSET_PATH)) {
      window.LOCALDRAW_ASSET_PATH.forEach((path) => {
