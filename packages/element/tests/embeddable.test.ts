...
describe("Embed sandbox policy", () => {
  it("grants same-origin sandbox only to domains in the dedicated allowlist", () => {
    const youtube = getEmbedLink("https://www.youtube.com/watch?v=dQw4w9WgXcQ");
    const gist = getEmbedLink("https://gist.github.com/octocat/123456");

    expect(youtube?.sandbox.allowSameOrigin).toBe(true);
    expect(gist?.sandbox.allowSameOrigin).toBe(false);
  });
});
