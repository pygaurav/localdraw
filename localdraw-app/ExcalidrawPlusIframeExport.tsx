...
import { loadActiveScene } from "./data/diskStorage";
    ...
    const handleMessage = async (event: MessageEvent<MESSAGE_FROM_PLUS>) => {
      ...
      if (event.data.type === EVENT_REQUEST_SCENE) {
        ...
        try {
          ...
          // Load scene from disk storage server (replaces localStorage)
          const { sceneData } = await loadActiveScene();
          const parsedSceneData: MESSAGE_SCENE_DATA = await parseSceneData({
            rawAppStateString: sceneData
              ? JSON.stringify(sceneData.appState)
              : null,
            rawElementsString: sceneData
              ? JSON.stringify(sceneData.elements)
              : null,
          });
